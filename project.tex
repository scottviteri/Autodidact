...
\section{Softmax-Relaxed Langevin-RAG Curriculum Selection}\label{sec:langevinrag}
...
\end{equation}
where $W_e \in \mathbb{R}^{V \times d}$ is the frozen token embedding matrix. This is a plain softmax---no Gumbel noise is added, because the gradient path $\boldsymbol{\lambda} \to \text{softmax} \to \tilde{\mathbf{e}} \to \text{transformer} \to Q$ is fully differentiable and we never need to backpropagate through a discrete sampling or argmax operation. The soft embedding $\tilde{\mathbf{e}}_i$ is a convex combination of real token embeddings, weighted by the softmax probabilities. When $\tau_t$ is large, $\boldsymbol{\pi}_i$ is diffuse and $\tilde{\mathbf{e}}_i$ is a broad average; when $\tau_t$ is small, $\boldsymbol{\pi}_i$ concentrates on one token and $\tilde{\mathbf{e}}_i \approx W_e[v^*]$ for the dominant token $v^*$.
...
\paragraph{Temperature annealing.} The softmax temperature $\tau_t$ is annealed linearly from $\tau_{\text{start}}$ (high, exploratory) to $\tau_{\text{end}}$ (low, sharp) over the SGLD steps:
...
\paragraph{Non-stationarity.} Because $\tau_t$ changes the energy landscape at each step, the SGLD chain does not converge to a stationary Boltzmann distribution. The procedure is better understood as \emph{optimisation with injected noise} (gradient ascent on Q, regularised by Langevin noise for diversity) rather than as approximate posterior sampling. The noise prevents the chains from collapsing to a single mode, while the annealing schedule progressively sharpens the softmax so that the final logits represent well-defined token choices.
...
\paragraph{Collection.} All $T_{\max}$ SGLD steps run to completion, then the final logits from each of the $K$ chains are collected, yielding $K$ samples (one per chain). The number of SGLD steps serves as a burn-in period: enough steps for the chains to climb the Q-landscape and for the softmax temperature to anneal to $\tau_{\text{end}}$.
...
\paragraph{Index construction and rolling refresh.} At startup we stream through the training corpus, extract $I$ fixed-length token windows, embed them with a pretrained sentence-transformer (all-MiniLM-L6-v2, $d_{\text{st}} = 384$, normalised), and store the resulting vectors in a FAISS \texttt{IndexFlatIP} (flat brute-force inner product---no tree or quantisation). The embeddings, token IDs, and decoded texts are held in a fixed-size \emph{ring buffer} of capacity~$I$.

Each training step, $n_{\text{ingest}}$ fresh windows are consumed from the data stream, embedded, and written into the ring buffer at the current write head, overwriting the oldest entries. The FAISS index is then rebuilt from the updated embedding matrix. Because \texttt{IndexFlatIP} is just a dense matrix, rebuilding for $I = 50{,}000$ vectors of dimension~384 amounts to a single \texttt{memcpy} ($< 5$\,ms). With $n_{\text{ingest}} = 100$ and $I = 50{,}000$, each step replaces $0.2\%$ of the index. The full buffer turns over after 500 steps---the same total throughput as a bulk refresh every 500 steps, but spread smoothly to avoid the LM loss jumps caused by a sudden distribution shift.

\paragraph{Query and retrieval (each step).} The $K$ discrete query sequences from the token extraction step are decoded to text and embedded with the same sentence-transformer. For each query, the top-$k$ nearest neighbours in the FAISS index are retrieved by cosine similarity. By default, one result is \emph{softmax-sampled} from the top-$k$ per query rather than taking the deterministic top-1:
\begin{equation}\label{eq:rag_sample}
    p_j \propto \exp\!\bigl(\text{score}_j \,/\, \tau_{\text{rag}}\bigr), \qquad j = 1, \ldots, k,
\end{equation}
where $\tau_{\text{rag}}$ is a sampling temperature (default 0.1). This gives diversity: instead of every query selecting its single nearest neighbour (which often overlap), each query draws from its local neighbourhood, producing a more varied retrieved set. Results are deduplicated across all queries to produce $R \leq K$ unique training examples.

\subsection{Softmax-Relaxed Langevin-RAG Algorithm}\label{sec:langevin_algorithm}

\begin{algorithm}[h]
\caption{Curriculum Selection via Softmax-Relaxed Langevin-RAG}
\begin{algorithmic}[1]
\State \textbf{Initialise:} LM parameters $\theta$; Q-network $\phi$; target $\bar{\phi} \gets \phi$
\State Build FAISS ring buffer of capacity $I$ from first $I$ corpus windows (Section~\ref{sec:rag})
\Statex
\For{$k = 0, 1, 2, \ldots$}
    \Statex
    \State \textcolor{gray}{\textit{// Stage 1: Softmax-relaxed SGLD in logit space}}
    ...
    \EndFor
    \State Collect final logits $\boldsymbol{\lambda}^{(1)}_{T_{\max}}, \ldots, \boldsymbol{\lambda}^{(K)}_{T_{\max}}$ \Comment{One sample per chain}
    \Statex
    ...
    \Statex
    \State \textcolor{gray}{\textit{// Stage 3: RAG retrieval + rolling refresh}}
    \State Embed query texts $\{\text{decode}(\mathbf{t})\}$ with sentence-transformer
    \State Retrieve $R$ unique training examples via softmax-sampling from top-$k$ (Eq.~\eqref{eq:rag_sample})
    \State Ingest $n_{\text{ingest}}$ fresh windows into ring buffer; rebuild FAISS index
    \Statex
...
Both the discrete-Q and Langevin-RAG pipelines can be understood through a unified free-energy lens. In the discrete-Q mode, the Boltzmann policy over a finite candidate set $C_k$ is the exact solution to a free-energy minimisation problem at each state:
\begin{equation}
...
In the Langevin-RAG mode, SGLD approximately samples from the Boltzmann distribution $p(\boldsymbol{\lambda}) \propto \exp(E(\boldsymbol{\lambda}))$ over the continuous logit space. The Langevin noise plays the role of the entropy term---it prevents the chains from collapsing to a single mode, maintaining diversity among the $K$ samples. However, because the softmax temperature $\tau_t$ is annealed during the run, the energy landscape is non-stationary and the chain does not converge to a fixed Boltzmann distribution. In practice the procedure functions as Q-guided optimisation with noise-injected diversity, rather than as exact posterior sampling.
